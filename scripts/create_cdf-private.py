...
PRIVATE-log version of create_cdf.py.  Same plots, but reading the
private-results/ layout, where each run directory holds

    solver-<job>-<run>   solver stdout+stderr, lines prefixed "<cpu>/<wall>\\t"
    vars-<job>-<run>     runsolver's key=value result file

instead of stdout.log / varfile.log, and where the wall-clock limit is 3700 s
(the binding limit being 3600 s of CPU).  Tracks are discovered from the
directory names rather than hard-coded, since the private set has no track 5.

For every track we draw two curves - the "exact" and the "approx" run - on the
same axes:

    x axis : solve time in seconds        (LOG scale)
    y axis : number of instances solved   (LINEAR scale)

An instance counts as SOLVED when its solver log has a `c s log10-estimate`
line AND a final-count line:
...
The solve time is runsolver's wall-clock time (WCTIME from vars-*), which is
...
def log_file(run_dir: Path, prefix: str) -> Path | None:
    """The single solver-/vars- file in a run directory (names carry job ids)."""
...
def wctime(run_dir: Path) -> float | None:
    """runsolver wall-clock time (seconds) for a run, from vars-*."""
    var = log_file(run_dir, "vars")
    if var is None:
        return None
...
def is_solved(run_dir: Path, suite: str) -> bool:
    """Solved = has a log10-estimate line AND a final-count line for the suite."""
    solver = log_file(run_dir, "solver")
    if solver is None:
        return False
    # strip runsolver's per-line "<cpu>/<wall>\t" timestamps before matching
    text = TS_RE.sub("", solver.read_text(errors="replace"))
...
def plot_track(track: int) -> None:
    ...
    ax.set_title(f"Ganak CDF - Track {track} private  (exact vs approx)")
